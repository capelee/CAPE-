...
async function checkId(id: string, name: string) {
  ...
  try {
    ...
    fs.appendFileSync('remaining_descriptions.txt', `=== DESC FOR ${name} (${id}) ===\n`);
    ...
    fs.appendFileSync('remaining_descriptions.txt', `${resultText}\n\n`);
    console.log(`Finished ${name}`);
  } catch (err: any) {
    fs.appendFileSync('remaining_descriptions.txt', `[${name}] Error: ${err.message}\n\n`);
    console.log(`Error on ${name}:`, err.message);
  }
}

async function run() {
  fs.writeFileSync('remaining_descriptions.txt', '=== REMAINING IMAGE DESCRIPTIONS ===\n\n');
